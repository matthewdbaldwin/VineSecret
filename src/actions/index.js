...
function hydrateLocalCart(items) {
    return items
...
function calculateCart(items) {
    const hydrated = hydrateLocalCart(items);
...
function syncLocalCart(items, dispatch) {
...
function loadLocalCart(dispatch) {
    ...
    return syncLocalCart(items, dispatch);
}
...
export function addItemToCart(productId, quantity) {
    return async function dispatchAddItem(dispatch) {
        ...
        const localCart = syncLocalCart(currentItems, dispatch);
        ...
        try {
            const response = await axios.post(`/api/cart/items/${productId}`, { quantity }, getCartConfig());
...
export function updateLocalCartItem(productId, quantity) {
    return function dispatchUpdateLocal(dispatch) {
        ...
        syncLocalCart(filteredItems, dispatch);
    };
}
